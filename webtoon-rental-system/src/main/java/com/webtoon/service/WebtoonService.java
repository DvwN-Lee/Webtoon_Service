...
public class WebtoonService {
    ...
    private final WebtoonRepository webtoonRepository;
    private final EpisodeRepository episodeRepository;
    private final NotificationService notificationService; // 과제 스펙에 따라 유지
    ...
    /**
     * 새 웹툰 생성 (id는 Long으로 생성)
     */
    public Webtoon createWebtoon(String title, String authorId) {
        Webtoon webtoon = new Webtoon();

        // UUID 기반으로 Long ID 생성 (간단한 데모용)
        Long webtoonId = Math.abs(UUID.randomUUID().getMostSignificantBits());
        webtoon.setId(webtoonId);   // Webtoon.setId(Long) 기준
    ...
    public Episode publishEpisode(Long webtoonId, String title, String content,
        ...
        Long episodeId = Math.abs(UUID.randomUUID().getMostSignificantBits());
        ...
        // Observer 패턴으로 충분하면 이 줄은 주석 처리해도 됨
        // notificationService.notifyNewEpisode(webtoon, episode);
